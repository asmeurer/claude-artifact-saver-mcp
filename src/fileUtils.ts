...
export const saveArtifactToFile = async (artifact: Artifact): Promise<string> => {
  ...
  if (artifact.title) {
    ...
    if (artifact.title.includes('/')) {
      ...
      outputPath = path.join(config.savePath, sanitizeFileName(`${artifact.title}.${extension}`));
    }
  } else {
    ...
    outputPath = path.join(
      config.savePath,
      sanitizeFileName(
        `artifact-${artifact.id}.${getExtensionForType(artifact.type, artifact.language)}`,
      ),
    );
  }
